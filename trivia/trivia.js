...
function Trivia() {
	...
	this.setCateg = function(objf, type) {
		...
		if(document.getElementById('tcateg')) document.getElementById('tcateg').innerHTML = 'Trivia Category: <select style="display:none" '+ onchg +'>'+ tcateg +'</select>';
	}
	...
	this.setQuizXML = function(propf) {
		...
		if(document.getElementById('ntotalq2')) document.getElementById('ntotalq2').innerHTML = nquizzes;
	}
	...
	this.sQuiz = function(stn) {
		...
		if(document.getElementById('quiz')) {
			...
			document.getElementById('quiz').innerHTML = addct +'<h4 id="qnr">Question # '+ (nquiz + 1) +'</h4><h4 id="tquestion">&#9675; '+ quizzes[nquiz]['que'] +'</h4><div id="qansw">'+ (level == 'level1' ? qAnswerL1() : qAnswerL2()) +'</div>';
	...
	this.resetTrivia = function() {
		...
		document.getElementById('quiz').innerHTML = '<a href="#test" data-role="button" data-theme="a">Begin Quiz</a>';
	}
	...
	var qAnswerL1 = function() {
		...
		this.getAnswer = function(answ){
			...
			if(document.getElementById('qansw')) {
				...
				seeansw += ((qmode == 'qindex' && nquiz == (nquizzes - 1)) || (qmode == 'qrandom' && iquizzes.length == 1)) ? '<h4> - <em>That was the last quiz</em>.</h4>' : '<div id="nextq" onclick="obTrivia.sQuiz(\'next\')">Next</div>';
	...
	var files_xml = {'General':'general.xml'};
